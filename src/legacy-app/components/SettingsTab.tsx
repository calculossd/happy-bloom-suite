  ...
  return (
    <div className="space-y-6" id="settings_tab_container">
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        {/* BLUEPRINT & MANUAL DE CLONAGEM CARD (NOVO) */}
        <div className="p-6 bg-[#121215] border border-white/10 rounded-2xl flex flex-col justify-between space-y-4 relative overflow-hidden">
          {/* Decorative glowing gradient circle */}
          <div className="absolute -right-10 -top-10 w-32 h-32 bg-[var(--brand-primary)]/10 rounded-full blur-2xl pointer-events-none" />
          
          <div className="space-y-2 relative z-10">
            <span className="text-[10px] uppercase tracking-wider font-extrabold text-[var(--brand-primary)] bg-[var(--brand-primary)]/10 px-2 py-0.5 rounded border border-[var(--brand-primary)]/25 inline-block font-mono animate-pulse">Homologado</span>
            <h3 className="text-base font-bold text-[#F1F4EE] flex items-center gap-1.5 flex-wrap font-sans">
              <FileText className="h-4.5 w-4.5 text-[var(--brand-primary)]" />
              Blueprint & Especificação Técnica de Clonagem (PDF)
              <button
                type="button"
                onClick={() => triggerHelp('Manual de Clonagem', 'Gere instantaneamente um documento técnico detalhado de todas as abas, botões e arquiteturas do sistema como especificação idônea de duplicação total sem necessidade de visual prévio.')}
                className="text-[#8BA58D] hover:text-[var(--brand-primary)] bg-[var(--brand-primary)]/5 p-1 rounded-lg border border-[var(--brand-primary)]/10 hover:border-[var(--brand-primary)]/20 transition cursor-pointer"
                title="Ver Explicação"
              >
                <HelpCircle className="h-3.5 w-3.5" />
            ...
            <p className="text-xs text-[#8E8E93] leading-relaxed font-sans">
              Crie um documento PDF premium e extremamente completo, mapeando <strong>botão-a-botão</strong>, cada caixa de formulário, campos de orçamentos e toda a estruturação de engenharia reversa do <strong>{brandConfig.name}</strong> para que qualquer desenvolvedor possa clonar este site com perfeição absoluta.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-2 relative z-10">
            <button
              ...
              className="py-2.5 bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 hover:border-zinc-700 text-zinc-300 rounded-xl text-xs font-semibold transition flex items-center justify-center gap-1.5 cursor-pointer font-sans duration-200"
            >
              <Download className="h-3.5 w-3.5 text-zinc-400" />
              Relatório Resumido do Ateliê
            </button>

            <button
              onClick={handleGenerateBlueprintPDF}
              className="py-2.5 bg-[var(--brand-primary)]/10 hover:bg-[var(--brand-primary)]/20 border border-[var(--brand-primary)]/25 text-[var(--brand-primary)] rounded-xl text-xs font-bold transition flex items-center justify-center gap-1.5 cursor-pointer font-sans duration-200 shadow-[0_4px_12px_rgba(244,110,31,0.05)] active:scale-98"
              id="btn_generate_blueprint_pdf"
            >
              <Sparkles className="h-3.5 w-3.5 animate-pulse text-[var(--brand-primary)]" />
              Gerar Manual de Clonagem (PDF)
            </button>
          </div>
        </div>
