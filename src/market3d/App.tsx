...
export default function App() {
  ...
  return (
    <div className="bg-[#03060A] text-[#EDF2F7] min-h-screen font-sans selection:bg-orange-500/30 selection:text-orange-100 flex flex-col justify-between overflow-x-hidden antialiased" id="intel-applet-root">
      ...
      {/* LUXURY TOP BRAND STRIP */}
      <nav className="border-b border-[#141A26] bg-[#070C16]/80 backdrop-blur-xl sticky top-0 z-40 px-6 py-4.5">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          
          {/* Logo with high-end editorial tracking */}
          <div className="flex items-center gap-3">
            <div className="h-8.5 w-8.5 rounded-lg bg-gradient-to-br from-orange-500 to-amber-600 flex items-center justify-center shadow-lg shadow-orange-500/20 text-white font-black text-xs tracking-tight">
              3D
            </div>
            <div>
              <h1 className="font-display font-medium text-slate-100 text-sm tracking-[0.06em] uppercase">
                Pesquisa de Mercado <span className="text-orange-500 font-bold font-sans">·</span> Intel
              </h1>
              <p className="text-[10px] text-slate-500 tracking-wider font-mono font-medium flex items-center gap-1.5 mt-0.5">
          ...
          <div className="hidden lg:flex items-center gap-8 text-xs text-slate-400 font-medium font-mono">
            <div className="flex flex-col">
              <span className="text-[9px] text-slate-500 uppercase tracking-widest font-sans font-bold">Base de Moldes</span>
              <span className="text-slate-100 mt-0.5">3.000 Cadastrados</span>
            </div>
            <div className="h-6 w-px bg-slate-800" />
            <div className="flex flex-col">
              <span className="text-[9px] text-slate-500 uppercase tracking-widest font-sans font-bold">Escopo Operacional</span>
              <span className="text-slate-100 mt-0.5">Zero Redundâncias</span>
            </div>
            <div className="h-6 w-px bg-slate-800" />
            <div className="flex flex-col">
              <span className="text-[9px] text-slate-500 uppercase tracking-widest font-sans font-bold">Tecnologia</span>
              <span className="text-orange-400 mt-0.5">FDM Real</span>
            </div>
          </div>

          {/* Dynamic Marketplace Selection & API keys panel */}
          <div className="flex items-center gap-3 shrink-0">
            {/* APIs Config button */}
            <button
              onClick={() => setIsApiSettingsOpen(true)}
              className="flex items-center gap-1.5 bg-[#0C101B] hover:bg-[#151D31] px-3.5 py-1.5 rounded-xl border border-[#171D2D] text-[10px] font-bold uppercase tracking-wider text-slate-350 hover:text-orange-400 cursor-pointer transition-all shrink-0 h-[34px]"
              title="Configurar chaves de APIs para dados reais"
            >
              <Key className="w-3.5 h-3.5 text-orange-400" />
              <span className="hidden sm:inline">APIs</span>
              <span className={`h-1.5 w-1.5 rounded-full ${
                (serverKeysStatus.GEMINI_API_KEY || customKeys.GEMINI_API_KEY)
                  ? 'bg-emerald-500 animate-pulse'
                  : 'bg-rose-500'
              }`} />
            </button>

            <div className="flex items-center gap-1.5 bg-[#0C101B] p-1 rounded-xl border border-[#171D2D] shrink-0 h-[34px]">
              {(['shopee', 'mercadolivre', 'amazon'] as const).map((plat) => {
                ...
                const colors = {
                  shopee: active ? 'bg-[#FF4500] text-white shadow-md' : 'text-slate-400 hover:text-slate-200 hover:bg-[#121827]',
                  mercadolivre: active ? 'bg-[#FFE600] text-slate-950 font-bold' : 'text-slate-400 hover:text-slate-200 hover:bg-[#121827]',
                  amazon: active ? 'bg-[#FF9900] text-slate-950 font-bold' : 'text-slate-400 hover:text-slate-200 hover:bg-[#121827]'
                };
                return (
                  <button
                    key={plat}
                    onClick={() => setSelectedPlatform(plat)}
                    className={`py-1 px-2.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all duration-200 cursor-pointer ${colors[plat]}`}
                  >
                    {titles[plat]}
                  </button>
                );
